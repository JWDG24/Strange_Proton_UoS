...
# (Loops over multiple weight indices)
# ============================================================
...
# ------------------------------------------------------------
# List of weight indices to process
# ------------------------------------------------------------
selected_weights = [0, 292, 293, 314, 315]  # <-- add as many as you want
...
PLOTS_DIR = os.path.join(BASE_DIR, "plots_weights")
...
for selected_weight in selected_weights:
    ...
    print(f"\nProcessing weight index {selected_weight}")

    pattern = os.path.join(OUTPUTS_DIR, f"output_w{selected_weight}_*.root")
    files = glob.glob(pattern)

    if not files:
        print("  No files found for this weight.")
        continue
    ...
    for histo in histo_titles:
        ...
        for particle, parton in valid_pairs:
            ...
            ratio = h_parton.Clone(
                f"ratio_{parton}_over_{particle}_w{selected_weight}"
            )
        ...
        out_base = os.path.join(
            PLOTS_DIR,
            f"ratio_{histo}_w{selected_weight}"
        )

        canvas.SaveAs(out_base + ".png")
        canvas.SaveAs(out_base + ".root")

print("\nAll ratio plots complete for all weight indices.")
